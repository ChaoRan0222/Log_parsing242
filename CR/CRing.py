...
patterns = {
    ...
    'Object': r'变更管理\(CR\) #(\d+):\s*([^<\n]+)',  # 匹配任务编号和描述
    'Test Spec Link / 测试用例链接':r'\*?\s*Test Spec Link / 测试用例链接:\s*(.*?)\s*(?=\*|$)',
    'ECR Number':r'\*?\s*ECR Number:\s*(.*?)\s*(?=\*|$)',
    'Baseline Ver. / 基线版本':r'\*?\s*Baseline Ver. / 基线版本:\s*(.*?)\s*(?=\*|$)',
    'Design Spec Link / 设计文档链接':r'\*?\s*Design Spec Link / 设计文档链接:\s*(.*?)\s*(?=\*|$)',
...
columns_mapping = {
    ...
    'Baseline Ver. / 基线版本':'Baseline Ver. / 基线版本',
    'Design Spec Link / 设计文档链接':'Design Spec Link / 设计文档链接',
...
# 保存 CSV 文件
df_latest.to_csv('t1e_demestic_202401-202501allmsg_utf8CRing.csv', index=False, encoding='utf-8', date_format='%Y-%m-%d')
