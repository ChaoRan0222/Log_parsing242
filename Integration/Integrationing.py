...
patterns = {
    ...
    'Object': r'Integration #(\d+):\s*([^<]+)',  # 匹配任务编号和描述
    'Test Spec Link / 测试用例链接':r'\*?\s*Test Spec Link / 测试用例链接:\s*(.*?)\s*(?=\*|$)',
    'Verification Report / 验证报告': r'\*?\s*Verification Report / 验证报告:\s*(https?://[^\s]*)',
    'ReleaseType / 释放类型':r'\*?\s*ReleaseType / 释放类型:\s*(.*?)\s*(?=\*|$)',
    'SWP Link':r'\*?\s*SWP Link:\s*(https?://[^\s]*)',
    'QAC Report Link':r'\*?\s*QAC Report Link:\s*(https?://[^\s]*)',
    'Hardware Version / 硬件版本':r'\*?\s*Hardware Version / 硬件版本:\s*(.*?)\s*(?=\*|$)',
    ...
    'Branch / Gerrit分支': r'\*?\s*Branch / Gerrit分支:\s*(.*?)\s*(?=\*|$)',
...
columns_mapping = {
    ...
    'Branch / Gerrit分支':'Branch / Gerrit分支',
    'Test Spec Link / 测试用例链接': 'Test Spec Link / 测试用例链接',
    'Verification Report / 验证报告': 'Verification Report / 验证报告',
    'ReleaseType / 释放类型':'ReleaseType / 释放类型',
    'SWP Link':'SWP Link',
    'QAC Report Link':'QAC Report Link',
    'Hardware Version / 硬件版本':'Hardware Version / 硬件版本',
...
# 保存 CSV 文件
df_latest.to_csv('t1e_demestic_202401-202501allmsg_utf8Integrationing.csv', index=False, encoding='utf-8', date_format='%Y-%m-%d')
